...
const DOMAIN_FIRST_YEAR_MARKUP_USD = Math.max(0, Number(process.env.DOMAIN_FIRST_YEAR_MARKUP_USD || 0));
const DOMAIN_RENEWAL_MARKUP_USD = Math.max(0, Number(process.env.DOMAIN_RENEWAL_MARKUP_USD || 13));
const DOMAIN_RENEWAL_MARKUP_RATE = 0;
const domainRenewalMarkup = () => DOMAIN_RENEWAL_MARKUP_USD;
...
function walletVisibleBalances(wallet) {
  const balances = walletBalancesObject(wallet);
  const realCurrencies = new Set((wallet.transactions || []).filter(t => Number(t.amount) > 0 && ['credit','debit'].includes(t.type)).map(t => clean(t.currency || wallet.currency).toUpperCase()));
  const base = clean(wallet.currency || 'NGN').toUpperCase();
  realCurrencies.add(base);
  const out = {};
  for (const [currency, amount] of Object.entries(balances)) {
    if (realCurrencies.has(clean(currency).toUpperCase())) out[currency] = Number(amount || 0);
  }
  if (!Object.keys(out).length) out[base] = Number(balances[base] || wallet.balance || 0);
  return out;
}
...
async function resolveRate(from, to) {
  ...
  if (from === 'NGN' && to === 'USD' && rate >= 0.1) { const e = new Error('The live NGN/USD exchange rate is invalid. Conversion was blocked to protect your balance.'); e.status = 502; throw e; }
  if (from === 'USD' && to === 'NGN' && rate <= 10) { const e = new Error('The live USD/NGN exchange rate is invalid. Conversion was blocked to protect your balance.'); e.status = 502; throw e; }
  return rate;
}
...
async function settleProviderPayment(reference, providerData={}){
  ...
  }else if(payment.purpose==='fintech_service'){
    const serviceOrder=await FintechServiceOrder.findById(payment.fintechServiceOrder);
    if(!serviceOrder)return {applied:false,reason:'fintech_service_order_not_found'};
    const expected=Number(serviceOrder.paymentAmount||payment.grossAmount||0),received=Number(providerData.amount||0);
    if(received>0&&Math.abs(received-expected)>0.01){serviceOrder.status='payment_failed';serviceOrder.failureMessage='Payment amount did not match the order total.';await serviceOrder.save();payment.status='failed';await payment.save();return {applied:false,reason:'amount_mismatch'};}
    if(serviceOrder.status!=='fulfilled'){serviceOrder.status='payment_verified';serviceOrder.paymentVerifiedAt=new Date();await serviceOrder.save();try{await fulfillFintechServiceOrder(serviceOrder);}catch(error){serviceOrder.status='fulfillment_failed';serviceOrder.failureMessage=clean(error.message);await serviceOrder.save();}}
  }else if(payment.purpose==='order'){
    const order=await Order.findById(payment.order);
    if(!order)return {applied:false,reason:'order_not_found'};
    const expected=Number(order.paymentAmount||payment.grossAmount||0),received=Number(providerData.amount||0);
    if(received>0&&Math.abs(received-expected)>0.01){payment.status='failed';payment.metadata={...(payment.metadata||{}),amountMismatch:{expected,received}};await payment.save();order.status='payment_pending';order.domainProvisionStatus='failed';order.domainProvisionMessage='Payment amount did not match the order total; no domain was provisioned.';await order.save();return {applied:false,reason:'amount_mismatch'};}
    if(order.status!=='paid'){order.status='paid';order.paymentReference=reference;await order.save();}
    if(domainItemFromOrder(order)&&order.domainProvisionStatus!=='completed'){const user=await User.findById(order.user)||await User.findOne({email:order.customerEmail});if(user){try{await provisionPaidDomain(order,user);}catch(error){console.error('Domain provisioning after payment failed:',error.message);}}}
  }
...
function publicServiceResult(message, extra = {}) {
  return { message, ...extra };
}

function publicProviderError(error, fallback = 'The requested service could not be completed.') {
  const status = Number(error?.status || 502);
  return { status, body: { message: clean(error?.userMessage || error?.message || fallback) } };
}
...
async function searchDomainReseller(query, options = {}) {
  ...
  if (!options.noCache && cached && Date.now() - cached.time < DOMAIN_CACHE_MS) return cached.value;
...
async function provisionPaidDomain(order, user) {
  ...
  if (existing) {
    if (order.domainProvisionStatus !== 'completed') {
      order.domainProvisionStatus = 'completed';
      order.domainProvisionMessage = 'Domain registered successfully';
      await order.save();
    }
    return existing;
  }
  ...
  // Claim the provisioning job atomically. Paystack's webhook and the browser
  // verification request can arrive at nearly the same time; only one request
  // is allowed to call the registrar so the domain cannot be registered twice.
  const claimed = await Order.findOneAndUpdate(
    {
      _id: order._id,
      status: 'paid',
      domainProvisionStatus: { $in: ['not_started', 'failed', 'payment_verified'] }
    },
    { $set: { domainProvisionStatus: 'processing', domainProvisionMessage: 'Registering your domain…' } },
    { new: true }
  );
  if (!claimed) {
    const current = await Order.findById(order._id);
    const alreadyCreated = await ManagedDomain.findOne({ order: order._id });
    if (alreadyCreated) return alreadyCreated;
    if (current?.domainProvisionStatus === 'processing') {
      const err = new Error('Domain registration is already being processed.');
      err.status = 409;
      throw err;
    }
    throw Object.assign(new Error('The domain registration order is not ready for provisioning.'), { status: 409 });
  }
  order = claimed;
  ...
  try {
    ...
    // Always obtain a fresh provider quote/availability immediately before
    // registration. The customer price is derived from this same live quote
    // during checkout; the registration request itself is sent only to the
    // provider and never trusts the browser price.
    const liveSearch = await searchDomainReseller(domain, { limit: 1, noCache: true });
    const liveItem = (liveSearch.results || []).find(x => clean(x.domain).toLowerCase() === domain);
    if (!liveItem || liveItem.available === false) {
      const err = new Error('This domain is no longer available for registration.');
      err.status = 409;
      throw err;
    }
    const liveProviderPrice = Number(liveItem.price || 0);
    if (!(liveProviderPrice > 0)) {
      const err = new Error('The live domain provider did not return a valid registration price.');
      err.status = 502;
      throw err;
    }

    // Keep the live provider price on the order for an auditable record. The
    // registrar bills the reseller account at its provider price; there is no
    // supported reason to invent a price field in the registration API body.
    order.items = (order.items || []).map(existingItem => {
      if (existingItem === item || clean(existingItem.domain || existingItem.name).toLowerCase() === domain) {
        return {
          ...existingItem,
          providerPrice: liveProviderPrice,
          wholesalePrice: liveProviderPrice,
          firstYearPrice: liveProviderPrice,
          usdPrice: Number(existingItem.usdPrice || existingItem.price || liveProviderPrice),
          liveProviderPriceAtProvision: liveProviderPrice,
          liveProviderPriceCheckedAt: new Date()
        };
      }
      return existingItem;
    });
    ...
    let provider;
    let lastError;
    const attempts = Math.max(1, Math.min(3, Number(process.env.DOMAIN_REGISTRATION_ATTEMPTS || 2)));
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      try {
        provider = await domainNameApiRequest('POST', 'domains/register-with-contacts', registrationPayload);
        break;
      } catch (error) {
        lastError = error;
        const transient = [408, 429, 500, 502, 503, 504].includes(Number(error.status || 0));
        if (!transient || attempt >= attempts) break;
        await sleep(750 * attempt);
      }
    }
    if (!provider) throw lastError || new Error('Domain provider registration failed.');

    const managed = await ManagedDomain.create({
      user: user._id || user.id,
      order: order._id,
      domain,
      status: 'active',
      expiresAt: provider.expiryDate || provider.expiresAt || undefined,
      nameservers: item.nameservers || nameServers,
      providerReference: String(provider.reference || provider.orderId || provider.id || ''),
      providerResponse: provider
    });
    order.domainProvisionStatus = 'completed';
    order.domainProvisionMessage = providerMessage(provider, 'Domain registered successfully');
    await order.save();
    return managed;
  } catch (err) {
    order.domainProvisionStatus = 'failed';
    order.domainProvisionMessage = err.message;
    await order.save();
    throw err;
  }
}
...
app.post('/api/auth/signup',authRequestLimit,async (req, res, next) => {
  try {
    ...
    return res.status(201).json({ success:true, message: 'Signup successful. Create your dashboard PIN.', token: signToken(user), user: ['admin','staff'].includes(user.role) ? toPublicUser(user) : toCustomerUser(user), next: 'create-pin', accountType: requestedRole });
  } catch (error) {
...
app.post('/api/auth/emails', auth, async(req,res)=>{
  ...
  res.status(201).json({message:'Additional email added.',user:['admin','staff'].includes(user.role)?toPublicUser(user):toCustomerUser(user)});
});
app.delete('/api/auth/emails', auth, async(req,res)=>{
  ...
  await user.save({validateModifiedOnly:true});res.json({message:'Email removed.',user:['admin','staff'].includes(user.role)?toPublicUser(user):toCustomerUser(user)});
});
...
app.post('/api/auth/login',authRequestLimit,async (req, res, next) => {
  try {
    ...
    await ensureUnifiedResellerAccount(user, req.body || {});
    return res.json({ success:true, message: user.pinHash ? 'Login successful. Enter your PIN.' : 'Login successful. Create your PIN.', token: signToken(user), user: ['admin','staff'].includes(user.role)?toPublicUser(user):toCustomerUser(user), next: user.pinHash ? 'verify-pin' : 'create-pin' });
...
app.get('/api/auth/me', auth, async (req, res) => {
  ...
  return res.json({ success:true, user:['admin','staff'].includes(user.role)?toPublicUser(user):toCustomerUser(user) });
});
...
app.post('/api/auth/pin/create', auth, async (req, res) => {
  ...
  await user.save();
  res.json({ message: 'PIN created successfully. Dashboard access unlocked.', user: ['admin','staff'].includes(user.role)?toPublicUser(user):toCustomerUser(user), dashboardAccess: true });
});
...
app.post('/api/auth/pin/verify', auth, async (req, res) => {
  ...
  res.json({ message: 'PIN verified. Dashboard access unlocked.', user: ['admin','staff'].includes(user.role)?toPublicUser(user):toCustomerUser(user), dashboardAccess: true });
});

app.get('/api/user/dashboard', auth, async (req, res) => {
  const user = await User.findById(req.user.id).select('-passwordHash -pinHash -adminNote -staffPermissions -riskStatus');
  if (!user) return res.status(404).json({ message: 'User not found' });
  const wallet = await getOrCreateWallet(user);
  ...
  res.json({ user: toCustomerUser(user), domains, wallet: { balance: walletAmount(wallet, wallet.currency), currency: wallet.currency, transactions: wallet.transactions.slice(-10).reverse() }, orders, summary: { orders: orders.length, supportStatus: 'Active', balance: walletAmount(wallet, wallet.currency), currency: wallet.currency } });
});
...
app.get('/api/reseller/api-projects',auth,requireReseller,async(req,res)=>{
  ...
  if(product && !(profile.apiProjects||[]).some(item=>item.product===product)){
    profile.apiProjects.push({name:product==='domain'?'Domain API':'Banking API',website:'',product,scopes:product==='domain'?['domains:read','domains:write','domains:renew']:['banking:read','payments:create','accounts:create','payouts:create','webhooks:write']});
    await profile.save();
  }
...
app.post('/api/reseller/api-projects',auth,requireReseller,async(req,res)=>{
  ...
  profile.apiProjects.push({name,website:'',product,scopes:product==='domain'?['domains:read','domains:write','domains:renew']:['banking:read','payments:create','accounts:create','payouts:create','webhooks:write']});await profile.save();
...
app.patch('/api/reseller/api-projects/:projectId/settings',auth,requireReseller,async(req,res)=>{
  ...
  const allowedScopes=project.product==='domain'?['domains:read','domains:write','domains:renew']:['banking:read','payments:create','accounts:create','payouts:create','webhooks:write'];
...
function developerRequiredScope(req,product){
  if(product==='domain'){
    if(req.method!=='GET' && req.path.includes('/register'))return 'domains:write';
    if(req.method!=='GET' && req.path.includes('/renew'))return 'domains:renew';
    return 'domains:read';
  }
...
app.get('/api/v1/domains/search',resellerApiKeyAuth,async(req,res)=>{
  ...
  try{
    ...
    res.json({...result,results,environment:req.resellerApi.environment,pricingRule:{resellerBasePrice:'provider-price-plus-configured-platform-fee',platformFeeRate:DOMAIN_RESELLER_API_PLATFORM_FEE_RATE,platformFeePercent:DOMAIN_RESELLER_API_PLATFORM_FEE_RATE*100,renewal:'provider-renewal-plus-configured-platform-fee',markupUSD:0,resellerMarkup:'chosen-by-reseller-on-their-own-platform'}});
  }catch(e){res.status(e.status||502).json({message:e.message,provider:e.payload})}
});
app.post('/api/v1/domains/register',resellerApiKeyAuth,async(req,res)=>{
  try{
    if(req.resellerApi.product!=='domain')return res.status(403).json({error:{code:'WRONG_API_PRODUCT',message:'Use a Domain API key.'}});
    const domain=clean(req.body?.domain||req.body?.name).toLowerCase();
    if(!validDomainName(domain))return res.status(400).json({error:{code:'INVALID_DOMAIN',message:'A valid full domain name is required.'}});
    const idempotency=clean(req.headers['idempotency-key']);if(!idempotency)return res.status(400).json({error:{code:'IDEMPOTENCY_REQUIRED',message:'Idempotency-Key header is required.'}});
    const reference=`WNH-DOM-${crypto.createHash('sha256').update(`${req.resellerApi.project._id}:${idempotency}`).digest('hex').slice(0,24)}`;
    const existing=await Order.findOne({paymentReference:reference,user:req.resellerApi.profile.user});
    if(existing){
      const managed=await ManagedDomain.findOne({order:existing._id});
      return res.status(200).json({reference,orderId:existing._id,status:existing.domainProvisionStatus,domain:managed||null,duplicate:true});
    }
    const liveSearch=await searchDomainReseller(domain,{limit:1});
    const liveItem=(liveSearch.results||[]).find(x=>clean(x.domain).toLowerCase()===domain);
    if(!liveItem||liveItem.available===false)return res.status(409).json({error:{code:'DOMAIN_UNAVAILABLE',message:'This domain is no longer available.'}});
    const providerPrice=Number(liveItem.price||0);if(!(providerPrice>0))return res.status(502).json({error:{code:'PROVIDER_PRICE_MISSING',message:'The domain provider did not return a valid price.'}});
    const platformFee=domainResellerApiFee(providerPrice),totalUSD=Number((providerPrice+platformFee).toFixed(2));
    const user=await User.findById(req.resellerApi.profile.user);if(!user)return res.status(404).json({error:{code:'RESELLER_NOT_FOUND',message:'Reseller account not found.'}});
    const wallet=await getOrCreateWallet(user),currency=clean(req.body?.currency||wallet.currency||'NGN').toUpperCase(),rate=await getRate('USD',currency),debit=Number((totalUSD*rate).toFixed(2));
    if(walletAmount(wallet,currency)<debit)return res.status(400).json({error:{code:'INSUFFICIENT_BALANCE',message:`Insufficient reseller balance. Required ${debit.toFixed(2)} ${currency}.`},required:debit,currency});
    const contact=req.body?.contact||{email:req.body?.customer_email||req.body?.email,phone:req.body?.phone,firstName:req.body?.first_name,lastName:req.body?.last_name,country:req.body?.country,address:req.body?.address,city:req.body?.city,postalCode:req.body?.postal_code};
    const item={...liveItem,type:'domain',domain,name:domain,period:Math.max(1,Number(req.body?.period||1)),providerPrice,wholesalePrice:providerPrice,firstYearPrice:providerPrice,usdPrice:totalUSD,price:totalUSD,platformFee,platformFeeRate:DOMAIN_RESELLER_API_PLATFORM_FEE_RATE,contact};
    const order=await Order.create({user:user._id,customerEmail:user.email,items:[item],subtotal:providerPrice,platformFee,total:totalUSD,currency:'USD',paymentCurrency:currency,exchangeRate:rate,paymentAmount:debit,status:'paid',paymentReference:reference,domainProvisionStatus:'processing'});
    changeWalletAmount(wallet,currency,-debit);
    wallet.transactions.push({type:'debit',amount:debit,currency,reference,description:`Domain API registration: ${domain} (provider ${providerPrice.toFixed(2)} USD + ${DOMAIN_RESELLER_API_PLATFORM_FEE_RATE*100}% WNH API fee)`,status:'completed'});
    await wallet.save();
    let managed=null;
    try{managed=await provisionPaidDomain(order,user);}catch(error){order.domainProvisionMessage=error.message;await order.save();return res.status(202).json({message:'Payment was completed but domain registration needs retry.',reference,orderId:order._id,status:'provisioning_failed',domain:null,error:error.message});}
    return res.status(201).json({message:'Domain registered successfully.',reference,orderId:order._id,status:'active',domain:managed,charged:debit,currency,providerPrice,worldNetHostingFee:platformFee});
  }catch(e){return res.status(e.status||502).json({error:{code:'DOMAIN_REGISTRATION_FAILED',message:e.message}});}
});

app.get('/api/v1/domains/:domain',resellerApiKeyAuth,async(req,res)=>{
  if(req.resellerApi.product!=='domain')return res.status(403).json({error:{code:'WRONG_API_PRODUCT',message:'Use a Domain API key.'}});
  const record=await ManagedDomain.findOne({user:req.resellerApi.profile.user,domain:clean(req.params.domain).toLowerCase()});
  if(!record)return res.status(404).json({error:{code:'DOMAIN_NOT_FOUND',message:'Domain not found in this reseller account.'}});
  res.json({domain:record});
});

app.post('/api/v1/domains/renew',resellerApiKeyAuth,async(req,res)=>{
  try{
    if(req.resellerApi.product!=='domain')return res.status(403).json({error:{code:'WRONG_API_PRODUCT',message:'Use a Domain API key.'}});
    const domain=clean(req.body?.domain||req.body?.name).toLowerCase(),period=Math.max(1,Number(req.body?.period||1)),idempotency=clean(req.headers['idempotency-key']);
    if(!validDomainName(domain))return res.status(400).json({error:{code:'INVALID_DOMAIN',message:'A valid domain is required.'}});
    if(!idempotency)return res.status(400).json({error:{code:'IDEMPOTENCY_REQUIRED',message:'Idempotency-Key header is required.'}});
    const reference=`WNH-DOM-RENEW-${crypto.createHash('sha256').update(`${req.resellerApi.project._id}:${idempotency}`).digest('hex').slice(0,24)}`;
    const existing=await Order.findOne({paymentReference:reference,user:req.resellerApi.profile.user});
    if(existing)return res.status(200).json({reference,orderId:existing._id,status:existing.domainProvisionStatus||existing.status,duplicate:true});
    const managed=await ManagedDomain.findOne({user:req.resellerApi.profile.user,domain});
    if(!managed)return res.status(404).json({error:{code:'DOMAIN_NOT_FOUND',message:'Domain not found in this reseller account.'}});
    const liveSearch=await searchDomainReseller(domain,{limit:1}),liveItem=(liveSearch.results||[]).find(x=>clean(x.domain).toLowerCase()===domain);
    const providerRenewal=Number(liveItem?.renewalPrice||liveItem?.renewPrice||liveItem?.price||0);if(!(providerRenewal>0))return res.status(502).json({error:{code:'RENEWAL_PRICE_MISSING',message:'The provider did not return a renewal price.'}});
    const platformFee=domainResellerApiFee(providerRenewal),totalUSD=Number((providerRenewal+platformFee).toFixed(2));
    const user=await User.findById(req.resellerApi.profile.user),wallet=await getOrCreateWallet(user),currency=clean(req.body?.currency||wallet.currency||'NGN').toUpperCase(),rate=await getRate('USD',currency),debit=Number((totalUSD*rate).toFixed(2));
    if(walletAmount(wallet,currency)<debit)return res.status(400).json({error:{code:'INSUFFICIENT_BALANCE',message:`Insufficient reseller balance. Required ${debit.toFixed(2)} ${currency}.`},required:debit,currency});
    const order=await Order.create({user:user._id,customerEmail:user.email,items:[{type:'domain_renewal',domain,period,providerRenewalPrice:providerRenewal,platformFee,usdPrice:totalUSD}],subtotal:providerRenewal,platformFee,total:totalUSD,currency:'USD',paymentCurrency:currency,exchangeRate:rate,paymentAmount:debit,status:'paid',paymentReference:reference,domainProvisionStatus:'processing'});
    changeWalletAmount(wallet,currency,-debit);wallet.transactions.push({type:'debit',amount:debit,currency,reference,description:`Domain API renewal: ${domain}`,status:'completed'});await wallet.save();
    const provider=await domainNameApiRequest('POST',domainEndpoint('RENEW','api/domain/renew'),{domainName:domain,period});
    managed.expiresAt=provider.expiryDate||provider.expiresAt||managed.expiresAt;managed.providerResponse=provider;await managed.save();
    order.domainProvisionStatus='completed';order.domainProvisionMessage=providerMessage(provider,'Domain renewed successfully.');await order.save();
    await queueDeveloperWebhook(req.resellerApi.profile._id,req.resellerApi.project._id,'domain.renewed',{reference,domain,period,status:'completed',amount:totalUSD,currency:'USD'}).catch(()=>{});
    res.status(201).json({message:'Domain renewed successfully.',reference,orderId:order._id,domain:managed,providerPrice:providerRenewal,worldNetHostingFee:platformFee,charged:debit,currency});
  }catch(e){res.status(e.status||502).json({error:{code:'DOMAIN_RENEWAL_FAILED',message:e.message}});}
});
...
app.get('/api/domains/search',publicApiRequestLimit,async (req, res) => {
  ...
  try {
    ...
    const priced={...apiResult,results:(apiResult.results||[]).map(item=>{
      ...
      return {domain:item.domain,available:Boolean(item.available),status:item.status||'available',premium:Boolean(item.premium),currency:item.currency||process.env.DOMAIN_CURRENCY||'USD',firstYearPrice,price:firstYearPrice,renewalPrice};
    })};
    res.json({ query, ...priced, resellerConfigured: domainApiConfigured(), pricingRule:{firstYear:'live-domain-price',renewal:'live-domain-renewal-price-plus-$13'} });
  } catch (err) {
    if (mongoose.connection.readyState === 1) {
    ...
    res.status(err.status || 502).json({ query, results: [], source: 'domain-search-error', message: 'Domain search is temporarily unavailable. Please try again shortly.' });
  }
});
...
app.get('/api/orders/:id', auth, async (req, res) => {
  try {
    ...
    const domain = await ManagedDomain.findOne({ order: order._id });
    res.json({
      order: {
        id: String(order._id), status: order.status, paymentReference: order.paymentReference,
        total: order.total, currency: order.currency, paymentAmount: order.paymentAmount,
        paymentCurrency: order.paymentCurrency, domainProvisionStatus: order.domainProvisionStatus,
        domainProvisionMessage: order.domainProvisionMessage,
        createdAt: order.createdAt, updatedAt: order.updatedAt
      },
      domain: domain ? { domain: domain.domain, status: domain.status, expiresAt: domain.expiresAt, nameservers: domain.nameservers, providerReference: domain.providerReference } : null
    });
  } catch (error) { res.status(error.status || 500).json({ message: error.message || 'Could not load order status.' }); }
});
...
app.post('/api/domains/register-paid-order', auth, async (req, res) => {
  ...
  } catch (err) { res.status(err.status || 502).json({ message: 'Domain registration could not be completed. Please try again or contact support.' }); }
});
...
app.get('/api/domains/:domain/dns', auth, requireOwnedDomain, async (req, res) => {
  try {
    ...
    res.json({ domain: req.params.domain, nameservers: req.managedDomain.nameservers || [], records: Array.isArray(records) ? records : [] });
  } catch (err) { res.status(err.status || 502).json({ message: 'DNS records could not be loaded. Please try again.' }); }
});
app.post('/api/domains/:domain/dns', auth, requireOwnedDomain, async (req, res) => {
  try {
    ...
    res.status(201).json({ message: `${record.type} record created successfully.`, record: provider.record || provider.data || provider });
  } catch (err) { res.status(err.status || 502).json({ message: 'DNS record could not be created. Please try again.' }); }
});
app.put('/api/domains/:domain/dns/:recordId', auth, requireOwnedDomain, async (req, res) => {
  try {
    ...
    res.json({ message: `${record.type} record updated successfully.`, record: provider.record || provider.data || provider });
  } catch (err) { res.status(err.status || 502).json({ message: 'DNS record could not be updated. Please try again.' }); }
});
app.delete('/api/domains/:domain/dns/:recordId', auth, requireOwnedDomain, async (req, res) => {
  ...
  } catch (err) { res.status(err.status || 502).json({ message: 'DNS record could not be deleted. Please try again.' }); }
});
app.put('/api/domains/:domain/nameservers', auth, requireOwnedDomain, async (req, res) => {
  try {
    ...
    res.json({ message: 'Nameservers updated successfully.', nameservers: result.nameServers || nameservers });
  } catch (err) {
    res.status(err.status || 502).json({ message: 'Nameserver update could not be completed. Please try again.' });
  }
});
...
function decryptSensitive(value){try{const [ivHex,tagHex,dataHex]=String(value||'').split(':');if(!ivHex||!tagHex||!dataHex)return '';const secret=crypto.createHash('sha256').update(String(process.env.WITHDRAWAL_ENCRYPTION_KEY||JWT_SECRET)).digest();const decipher=crypto.createDecipheriv('aes-256-gcm',secret,Buffer.from(ivHex,'hex'));decipher.setAuthTag(Buffer.from(tagHex,'hex'));return Buffer.concat([decipher.update(Buffer.from(dataHex,'hex')),decipher.final()]).toString('utf8');}catch{return '';}}
app.post('/api/domains/transfers',auth,async(req,res)=>{try{const domain=clean(req.body.domain).toLowerCase();const authCode=clean(req.body.authCode);const email=clean(req.body.email).toLowerCase();if(!validDomainName(domain))return res.status(400).json({message:'Enter a valid full domain name.'});if(!authCode||authCode.length<3)return res.status(400).json({message:'A valid authorization/EPP code is required.'});if(!req.body.consent)return res.status(400).json({message:'Ownership authorization confirmation is required.'});const provider=await domainNameApiRequest('POST',domainEndpoint('TRANSFER','api/domain/transfer'),{domainName:domain,authCode,contactEmail:email,period:Number(req.body.period||1)});const item=await DomainTransfer.create({user:req.user.id,type:'transfer-in',domain,email,authCodeEncrypted:encryptSensitive(authCode),status:'processing',providerReference:String(provider.reference||provider.orderId||provider.id||'')});res.status(201).json({message:providerMessage(provider,'Domain transfer started successfully.'),reference:String(item._id),status:item.status})}catch(e){res.status(e.status||502).json({message:e.message,provider:e.payload})}});

app.post('/api/domains/:domain/renew',auth,requireOwnedDomain,async(req,res)=>{try{const period=Math.max(1,Number(req.body.period||1));const provider=await domainNameApiRequest('POST',domainEndpoint('RENEW','api/domain/renew'),{domainName:req.params.domain,period});res.json({message:providerMessage(provider,'Domain renewed successfully.')})}catch(e){res.status(e.status||502).json({message:'Domain renewal could not be completed. Please try again.'})}});
app.put('/api/domains/:domain/lock',auth,requireOwnedDomain,async(req,res)=>{try{const locked=Boolean(req.body.locked);const provider=await domainNameApiRequest('PUT',domainEndpoint('LOCK','api/domain/lock'),{domainName:req.params.domain,isLocked:locked});req.managedDomain.locked=locked;await req.managedDomain.save();res.json({message:`Domain ${locked?'locked':'unlocked'} successfully.`,locked})}catch(e){res.status(e.status||502).json({message:'Domain lock update could not be completed. Please try again.'})}});
app.get('/api/domains/:domain/epp',auth,requireOwnedDomain,async(req,res)=>{try{const provider=await domainNameApiRequest('POST',domainEndpoint('EPP','api/domain/auth-code'),{domainName:req.params.domain});res.json({message:'EPP/auth code retrieved securely.',authCode:provider.authCode||provider.eppCode||provider.code||''})}catch(e){res.status(e.status||502).json({message:'The EPP/auth code could not be retrieved.'})}});
app.put('/api/domains/:domain/contact',auth,requireOwnedDomain,async(req,res)=>{try{const registrant=normalizeContact(req.body.contact||req.body,'Registrant');if(!registrant)return res.status(400).json({message:'Complete WHOIS contact details are required.'});const contacts=['Registrant','Admin','Tech','Billing'].map(type=>({...registrant,contactType:type}));const provider=await domainNameApiRequest('PUT',domainEndpoint('CONTACT','api/domain/contact'),{domainName:req.params.domain,contacts});res.json({message:providerMessage(provider,'WHOIS contacts updated successfully.')})}catch(e){res.status(e.status||502).json({message:e.message,provider:e.payload})}});
...
async function prepareCustomerOrderItems(rawItems = []) {
  ...
  for (const raw of rawItems) {
    ...
    if (type === 'domain' || item.domain || (item.name && /\.[a-z]{2,63}$/i.test(String(item.name)))) {
      const domain = clean(item.domain || item.name).toLowerCase();
      if (!validDomainName(domain)) throw Object.assign(new Error(`Invalid domain name: ${domain}`), { status: 400 });
      const liveSearch = await searchDomainReseller(domain, { limit: 1, noCache: true });
      const liveItem = (liveSearch.results || []).find(x => clean(x.domain).toLowerCase() === domain);
      if (!liveItem || liveItem.available === false) throw Object.assign(new Error(`${domain} is no longer available.`), { status: 409 });
      const providerPrice = Number(liveItem.price || 0);
      if (!(providerPrice > 0)) throw Object.assign(new Error(`The live provider did not return a valid price for ${domain}.`), { status: 502 });
      const providerRenewal = Number(liveItem.renewalPrice || liveItem.renewPrice || providerPrice);
      prepared.push({
        ...item,
        ...liveItem,
        type: 'domain',
        domain,
        name: domain,
        period: Math.max(1, Number(item.period || 1)),
        providerPrice,
        wholesalePrice: providerPrice,
        firstYearPrice: providerPrice,
        usdPrice: providerPrice,
        price: providerPrice,
        renewalProviderPrice: providerRenewal,
        renewalPrice: Number((providerRenewal + domainRenewalMarkup(providerRenewal)).toFixed(2)),
        firstYearMarkupUSD: 0,
        renewalMarkupRate: 0,
        renewalMarkupUSD: DOMAIN_RENEWAL_MARKUP_USD,
        platformFee: 0
      });
    } else {
      const price = Number(item.usdPrice ?? item.price ?? 0);
      if (!Number.isFinite(price) || price < 0) throw Object.assign(new Error('Invalid item price.'), { status: 400 });
      prepared.push({ ...item, usdPrice: Number(price.toFixed(2)), price: Number(price.toFixed(2)) });
    }
  }
  return prepared;
}

function nonDomainPlatformFeeRate(items, role) { const hasNonDomain=items.some(item=>clean(item.type).toLowerCase()!=='domain'); return hasNonDomain&&feeAppliesToRole(role)?USER_PLATFORM_FEE_RATE:0; }

function calculateCustomerOrderPricing(items, role) {
  const subtotalUSD = Number(items.reduce((sum, item) => sum + Number(item.usdPrice ?? item.price ?? 0), 0).toFixed(2));
  // Domain first-year registration is charged at the live provider price with no WNH markup.
  // Existing platform fees remain applicable to other non-domain products.
  const nonDomainSubtotal = Number(items.filter(item => clean(item.type).toLowerCase() !== 'domain')
    .reduce((sum, item) => sum + Number(item.usdPrice ?? item.price ?? 0), 0).toFixed(2));
  const platformFeeUSD = feeForRole(nonDomainSubtotal, role);
  return { subtotalUSD, platformFeeUSD, totalUSD: Number((subtotalUSD + platformFeeUSD).toFixed(2)) };
}
...
app.post('/api/payments/paystack/checkout',auth,async(req,res)=>{try{
  ...
  const items=await prepareCustomerOrderItems(req.body.items);
  const {subtotalUSD,platformFeeUSD,totalUSD}=calculateCustomerOrderPricing(items,req.user.role);
  const paymentCurrency=normalizeCurrency(req.body.currency||req.body.paymentCurrency||'NGN','NGN');
  if(!supportedPaystackCheckoutCurrencies().includes(paymentCurrency))return res.status(400).json({message:`Currency ${paymentCurrency} is not enabled for checkout.`});
  const exchangeRate=await getRate('USD',paymentCurrency),paymentAmount=Number((totalUSD*exchangeRate).toFixed(2));
  const order=await Order.create({
    user:req.user.id,customerEmail:req.user.email,items,subtotal:subtotalUSD,platformFee:platformFeeUSD,
    platformFeeRate:nonDomainPlatformFeeRate(items,req.user.role),total:totalUSD,currency:'USD',
    paymentCurrency,exchangeRate,paymentAmount,status:'payment_pending'
  });
  ...
  if(!redirectUrl)return res.status(500).json({message:'FRONTEND_URL is required for payment redirects.'});
  const payment=await ProviderPayment.create({reference,purpose:'order',user:req.user.id,order:order._id,amount:paymentAmount,grossAmount:paymentAmount,platformFee:platformFeeUSD,currency:paymentCurrency,status:'pending',metadata:{subtotalUSD,totalUSD,exchangeRate,domainFirstYearProviderPrice:items.filter(i=>i.type==='domain').reduce((n,i)=>n+Number(i.providerPrice||0),0)}});
  const result=await Paystack.createCheckout({amount:paymentAmount,currency:paymentCurrency,customer:{name:req.user.name||req.user.email,email:req.user.email},merchantReference:reference,redirectUrl,metadata:{purpose:'order',orderId:String(order._id),userId:req.user.id}});
  const data=result.data||result;payment.checkoutUrl=clean(data.link||data.checkoutUrl);payment.providerReference=clean(data.id||data.reference);await payment.save();
  res.status(201).json({message:'Checkout initialized.',data:{reference,link:payment.checkoutUrl},order_id:order._id,items,subtotalUSD,platformFeeUSD,platformFeeRate:nonDomainPlatformFeeRate(items,req.user.role),totalUSD,paymentAmount,paymentCurrency,exchangeRate});
...
app.get('/api/payments/paystack/verify/:reference',auth,publicApiRequestLimit,async(req,res)=>{try{
  const reference=clean(req.params.reference);let local=await ProviderPayment.findOne({reference,user:req.user.id});let order=await Order.findOne({paymentReference:reference,user:req.user.id});
  if(!local&&!order)return res.status(404).json({message:'Payment not found.'});
  ...
  local=await ProviderPayment.findOne({reference,user:req.user.id});
  order=order||await Order.findOne({paymentReference:reference,user:req.user.id});
  const managed=order?await ManagedDomain.findOne({order:order._id}):null;
  res.json({status:true,data:{status:local?.status||status,reference,currency:local?.currency||data.currency||'',amount:Number(local?.grossAmount||data.amount||0),orderId:order?String(order._id):'',orderStatus:order?.status||'',domainProvisionStatus:order?.domainProvisionStatus||'',domainProvisionMessage:order?.domainProvisionMessage||'',domain:managed?.domain||''},verifiedBy:'paystack-server'});
}catch(e){res.status(e.status||502).json({message:e.message});}});
...
app.put('/api/profile/kyc', auth, async (req,res)=>{
  try {
    const user=await User.findById(req.user.id).select('+stroWalletCustomerId'); if(!user)return res.status(404).json({message:'Customer account not found.'});
    const nin=clean(req.body.nin).replace(/\D/g,''); const bvn=clean(req.body.bvn).replace(/\D/g,'');
    if(nin && !/^\d{11}$/.test(nin))return res.status(422).json({message:'NIN must contain 11 digits.'});
    if(bvn && !/^\d{11}$/.test(bvn))return res.status(422).json({message:'BVN must contain 11 digits.'});
    if(!nin&&!bvn)return res.status(422).json({message:'Enter NIN or BVN for verification.'});
    user.kyc={...(user.kyc?.toObject?.()||user.kyc||{}),dob:normalizeDob(req.body.dob),address:clean(req.body.address),city:clean(req.body.city),state:clean(req.body.state),postalCode:clean(req.body.postalCode),country:clean(req.body.country||'NGA').toUpperCase(),ninEncrypted:nin?encryptSensitive(nin):(user.kyc?.ninEncrypted||''),bvnEncrypted:bvn?encryptSensitive(bvn):(user.kyc?.bvnEncrypted||''),verifiedAt:new Date()};
    await user.save({validateModifiedOnly:true});
    res.json({message:'Identity information saved securely.',kyc:{verified:true,hasNin:Boolean(nin||user.kyc?.ninEncrypted),hasBvn:Boolean(bvn||user.kyc?.bvnEncrypted),dob:user.kyc.dob,address:user.kyc.address,city:user.kyc.city,state:user.kyc.state,postalCode:user.kyc.postalCode,country:user.kyc.country}});
  } catch(e){res.status(e.status||500).json({message:e.message||'Identity information could not be saved.'});}
});

// -------------------- StroWallet fintech services --------------------
function fintechMarkupPercent() { return Math.max(0, Number(process.env.STROWALLET_MARKUP_PERCENT || 100)); }
function fintechCustomerPrice(providerCost) { return Number((Number(providerCost || 0) * (1 + fintechMarkupPercent() / 100)).toFixed(2)); }
function fintechCatalog() {
  const usCost = Math.max(0, Number(process.env.STROWALLET_US_CARD_COST_USD || 3));
  const ngnCost = Math.max(0, Number(process.env.STROWALLET_NAIRA_CARD_COST_NGN || 1000));
  const virtualCost = Math.max(0, Number(process.env.STROWALLET_VIRTUAL_ACCOUNT_COST_NGN || 0));
  return [
    { code:'us_card', name:'US Dollar Virtual Card', currency:'USD', customerPrice:fintechCustomerPrice(usCost), available:StroWallet.configured(), requiresKyc:true },
    { code:'naira_card', name:'Naira Virtual Card', currency:'NGN', customerPrice:fintechCustomerPrice(ngnCost), available:StroWallet.configured(), requiresKyc:true },
    { code:'virtual_account', name:'Virtual Account', currency:'NGN', customerPrice:fintechCustomerPrice(virtualCost), available:StroWallet.configured(), requiresKyc:false }
  ];
}
function findFintechProduct(code) { return fintechCatalog().find(x => x.code === clean(code).toLowerCase()); }
function normalizeDob(value='') {
  const v=clean(value); if(!/^\d{4}-\d{2}-\d{2}$/.test(v)) return '';
  const d=new Date(`${v}T00:00:00Z`); if(Number.isNaN(d.getTime())) return '';
  return `${String(d.getUTCMonth()+1).padStart(2,'0')}/${String(d.getUTCDate()).padStart(2,'0')}/${d.getUTCFullYear()}`;
}
function getUserKycPayload(user, body={}) {
  const fullName=clean(body.name||user.name); const parts=fullName.split(/\s+/).filter(Boolean);
  const firstName=clean(body.firstName||parts[0]); const lastName=clean(body.lastName||parts.slice(1).join(' ')||parts[0]);
  const dob=normalizeDob(body.dob||user.kyc?.dob||''); const phone=clean(body.phone||user.phone).replace(/^\+/,'');
  const address=clean(body.address||body.line1||user.kyc?.address); const city=clean(body.city||user.kyc?.city); const state=clean(body.state||user.kyc?.state); const postalCode=clean(body.postalCode||body.postal_code||user.kyc?.postalCode); const country=clean(body.country||user.kyc?.country||'NGA').toUpperCase();
  const nin=clean(body.nin).replace(/\D/g,'') || decryptSensitive(user.kyc?.ninEncrypted||''); const bvn=clean(body.bvn).replace(/\D/g,'') || decryptSensitive(user.kyc?.bvnEncrypted||'');
  if(!firstName||!lastName||!validEmailAddress(user.email)||!dob||!phone||!address||!city||!state||!postalCode) throw Object.assign(new Error('Complete your name, date of birth, phone and address before requesting a card.'),{status:400});
  if(nin && !/^\d{11}$/.test(nin)) throw Object.assign(new Error('NIN must contain 11 digits.'),{status:422});
  if(bvn && !/^\d{11}$/.test(bvn)) throw Object.assign(new Error('BVN must contain 11 digits.'),{status:422});
  if(!nin && !bvn) throw Object.assign(new Error('NIN or BVN verification is required. You do not need a StroWallet customer ID.'),{status:422});
  return {fullName,firstName,lastName,dob,phone,address,city,state,postalCode,country,nin,bvn};
}
function safeFintechOrder(order) {
  return {id:String(order._id),service:order.service,status:order.status,customerPrice:order.customerPrice,currency:order.currency,cardId:order.metadata?.cardId||'',lastFour:order.metadata?.lastFour||'',cardBrand:order.metadata?.cardBrand||'',accountNumber:order.metadata?.accountNumber||'',accountName:order.metadata?.accountName||'',bankName:order.metadata?.bankName||'',createdAt:order.createdAt,updatedAt:order.updatedAt};
}
async function fulfillFintechServiceOrder(order) {
  if(!order || !['payment_verified','fulfillment_failed','pending'].includes(order.status)) return order;
  if(order.status==='fulfilled') return order;
  const user=await User.findById(order.user).select('+stroWalletCustomerId'); if(!user) throw new Error('Customer account not found.');
  order.status='fulfilling'; await order.save();
  try {
    const payload=order.metadata?.request||{};
    let result;
    if(order.service==='us_card') {
      result=await StroWallet.createDollarCard({
        name:payload.name,first_name:payload.first_name,last_name:payload.last_name,dob:payload.dob,id_type:'national_id',id_number:payload.id_number,email:user.email,
        line1:payload.line1,city:payload.city,state:payload.state,postal_code:payload.postal_code,country:payload.country||'NGA',amount_usd:String(order.providerCost),phone:payload.phone,brand:payload.brand||'Visa'
      });
      order.metadata={...(order.metadata||{}),cardId:String(result?.cardId||result?.card_id||result?.data?.cardId||result?.data?.card_id||''),lastFour:String(result?.lastFour||result?.last_four||result?.data?.lastFour||result?.data?.last_four||''),cardBrand:String(result?.cardBrand||result?.brand||result?.data?.cardBrand||result?.data?.brand||'')};
    } else if(order.service==='virtual_account') {
      result=await StroWallet.createVirtualAccount({email:user.email,account_name:payload.account_name||user.name,phone:payload.phone,webhook_url:payload.webhook_url});
      order.metadata={...(order.metadata||{}),accountNumber:String(result?.account_number||result?.accountNumber||result?.data?.account_number||result?.data?.accountNumber||''),accountName:String(result?.account_name||result?.accountName||result?.data?.account_name||result?.data?.accountName||user.name),bankName:String(result?.bank_name||result?.bankName||result?.data?.bank_name||result?.data?.bankName||'')};
      const providerCustomerId=StroWallet.extractCustomerId(result); if(providerCustomerId) { user.stroWalletCustomerId=providerCustomerId; await user.save({validateModifiedOnly:true}); }
    } else if(order.service==='naira_card') {
      let customerId=clean(user.stroWalletCustomerId);
      if(!customerId) {
        // The documented Naira-card endpoint requires a provider customerId.
        // We do not ask the user for it. Try to obtain it from the provider's account-creation response first.
        const account=await StroWallet.createVirtualAccount({email:user.email,account_name:user.name,phone:payload.phone,webhook_url:payload.webhook_url});
        customerId=StroWallet.extractCustomerId(account);
        if(customerId){ user.stroWalletCustomerId=customerId; await user.save({validateModifiedOnly:true}); }
      }
      if(!customerId) throw Object.assign(new Error('The StroWallet Naira-card API requires a provider customer ID, but StroWallet has not returned one for this account. No customer ID is requested from the user.'),{status:502});
      result=await StroWallet.createNairaCard({customerId,type:payload.type||'virtual',brand:payload.brand||'Verve',provider:payload.provider||'black'});
      order.metadata={...(order.metadata||{}),cardId:String(result?.cardId||result?.card_id||result?.data?.cardId||result?.data?.card_id||''),lastFour:String(result?.lastFour||result?.last_four||result?.data?.lastFour||result?.data?.last_four||''),cardBrand:String(result?.cardBrand||result?.brand||result?.data?.cardBrand||result?.data?.brand||payload.brand||'')};
    } else throw new Error('Unsupported financial product.');
    order.providerReference=String(result?.reference||result?.providerReference||result?.id||result?.data?.reference||result?.data?.id||'');
    order.providerStatus=clean(result?.status||result?.data?.status||'success');
    order.status='fulfilled'; order.fulfilledAt=new Date(); order.failureMessage='';
    await order.save();
    return order;
  } catch(error) {
    order.status='fulfillment_failed'; order.failureMessage=clean(error.message||'Provider fulfillment failed.'); await order.save(); throw error;
  }
}

app.get('/api/fintech/catalog', auth, async (req,res)=>{const user=await User.findById(req.user.id).select('+stroWalletCustomerId');const products=fintechCatalog().map(p=>({...p,available:p.available && (p.code!=='naira_card'||Boolean(user?.stroWalletCustomerId))})).map(p=>({code:p.code,name:p.name,currency:p.currency,price:p.customerPrice,available:p.available,requiresKyc:p.requiresKyc}));res.json({products});});
app.get('/api/fintech/orders', auth, async (req,res)=>{const orders=await FintechServiceOrder.find({user:req.user.id}).sort({createdAt:-1}).limit(30);res.json({orders:orders.map(safeFintechOrder)});});

app.post('/api/fintech/paystack/checkout', auth, async (req,res)=>{
  try {
    if(['admin','staff'].includes(req.user.role)) return res.status(403).json({message:'Financial product checkout is available to customer accounts only.'});
    if(!Paystack.configured()) return res.status(503).json({message:'Paystack is not configured for customer checkout.'});
    if(!StroWallet.configured()) return res.status(503).json({message:'Financial services are not configured yet. Add the real StroWallet public key on the backend.'});
    const product=findFintechProduct(req.body.service);
    if(!product||!product.available) return res.status(400).json({message:'That financial product is not currently available.'});
    const user=await User.findById(req.user.id).select('+stroWalletCustomerId'); if(!user)return res.status(404).json({message:'Customer account not found.'});
    const kyc=product.code==='virtual_account'?{fullName:user.name,firstName:String(user.name||'').split(/\s+/)[0],lastName:String(user.name||'').split(/\s+/).slice(1).join(' ')||String(user.name||''),dob:'',phone:clean(user.phone).replace(/^\+/,''),address:'',city:'',state:'',postalCode:'',country:'NGA',nin:'',bvn:''}:getUserKycPayload(user,req.body);
    if(product.code==='virtual_account' && !kyc.phone) return res.status(400).json({message:'Add your phone number to your profile before creating a virtual account.'});
    const providerCost=product.code==='us_card'?Math.max(0,Number(process.env.STROWALLET_US_CARD_COST_USD||3)):product.code==='naira_card'?Math.max(0,Number(process.env.STROWALLET_NAIRA_CARD_COST_NGN||1000)):Math.max(0,Number(process.env.STROWALLET_VIRTUAL_ACCOUNT_COST_NGN||0));
    if(product.code==='naira_card' && !clean(user.stroWalletCustomerId)) {
      return res.status(503).json({message:'Naira Card onboarding is not enabled for this account yet. StroWallet requires a provider customer ID for this card endpoint; World Net Hosting will obtain it internally when StroWallet provides the supported onboarding flow. You do not need to enter a customer ID.'});
    }
    const customerPrice=product.customerPrice;
    const paymentCurrency=product.currency==='USD'?normalizeCurrency(req.body.paymentCurrency||'NGN','NGN'):'NGN';
    if(!supportedPaystackCheckoutCurrencies().includes(paymentCurrency))return res.status(400).json({message:`${paymentCurrency} checkout is not enabled.`});
    const rate=product.currency===paymentCurrency?1:await getRate(product.currency,paymentCurrency);
    const paymentAmount=Number((customerPrice*rate).toFixed(2));
    if(paymentAmount<=0) {
      const order=await FintechServiceOrder.create({user:user._id,service:product.code,providerCost,customerPrice,currency:product.currency,paymentCurrency,exchangeRate:rate,paymentAmount,status:'payment_verified',metadata:{request:{name:kyc.fullName,first_name:kyc.firstName,last_name:kyc.lastName,dob:kyc.dob,id_number:kyc.nin||kyc.bvn,line1:kyc.address,city:kyc.city,state:kyc.state,postal_code:kyc.postalCode,country:kyc.country,phone:kyc.phone,brand:req.body.brand,type:req.body.type,provider:req.body.provider,account_name:user.name,webhook_url:`${String(process.env.BACKEND_URL||'').replace(/\/$/,'')}/api/webhooks/strowallet`}}});
      await fulfillFintechServiceOrder(order);
      return res.status(201).json({message:'Product created successfully.',order:safeFintechOrder(order)});
    }
    const order=await FintechServiceOrder.create({user:user._id,service:product.code,providerCost,customerPrice,currency:product.currency,paymentCurrency,exchangeRate:rate,paymentAmount,status:'payment_pending',metadata:{request:{name:kyc.fullName,first_name:kyc.firstName,last_name:kyc.lastName,dob:kyc.dob,id_number:kyc.nin||kyc.bvn,line1:kyc.address,city:kyc.city,state:kyc.state,postal_code:kyc.postalCode,country:kyc.country,phone:kyc.phone,brand:req.body.brand,type:req.body.type,provider:req.body.provider,account_name:user.name,webhook_url:`${String(process.env.BACKEND_URL||'').replace(/\/$/,'')}/api/webhooks/strowallet`}}});
    const reference=`WNH-FINTECH-${order._id}`;order.paymentReference=reference;await order.save();
    const payment=await ProviderPayment.create({reference,purpose:'fintech_service',user:user._id,amount:paymentAmount,grossAmount:paymentAmount,currency:paymentCurrency,status:'pending',fintechServiceOrder:order._id,metadata:{service:product.code,customerPrice,providerCost,exchangeRate}});
    const redirectUrl=clean(req.body.redirectUrl)||`${String(process.env.FRONTEND_URL||'').replace(/\/$/,'')}/payment-success.html`;
    const result=await Paystack.createCheckout({amount:paymentAmount,currency:paymentCurrency,customer:{name:user.name,email:user.email},merchantReference:reference,redirectUrl,metadata:{purpose:'fintech_service',service:product.code,orderId:String(order._id),userId:String(user._id)}});
    const data=result.data||result;payment.checkoutUrl=clean(data.link||data.checkoutUrl||data.authorization_url);payment.providerReference=clean(data.id||data.reference);await payment.save();
    res.status(201).json({message:'Payment checkout initialized.',data:{reference,link:payment.checkoutUrl},order:safeFintechOrder(order),amount:paymentAmount,currency:paymentCurrency});
  } catch(error){const p=publicProviderError(error);res.status(p.status).json(p.body);}
});

app.get('/api/fintech/orders/:id', auth, async (req,res)=>{const order=await FintechServiceOrder.findOne({_id:req.params.id,user:req.user.id});if(!order)return res.status(404).json({message:'Financial product order not found.'});res.json({order:safeFintechOrder(order)});});

app.post('/api/webhooks/strowallet', async (req,res)=>{
  // StroWallet's current docs describe card webhooks for created/failed/transaction events.
  // Keep the response minimal and never echo provider payloads to customers.
  try {
    const event=clean(req.body?.event||Object.keys(req.body||{}).find(k=>k.startsWith('virtualcard.'))||'');
    const reference=clean(req.body?.reference);
    if(reference){const order=await FintechServiceOrder.findOne({providerReference:reference});if(order){order.providerStatus=clean(req.body.status||event);if(/failed|declined/i.test(event))order.status='provider_failed';await order.save();}}
...
app.get('/api/wallet', auth, async (req,res)=>{try{
  ...
  const balances=walletVisibleBalances(wallet);
  res.json({available_balance:walletAmount(wallet,wallet.currency),balance:walletAmount(wallet,wallet.currency),currency:wallet.currency,balances,ngn_balance:Number(balances.NGN||0),usd_balance:Number(balances.USD||0),balance_updated_at:wallet.updatedAt,user:toCustomerUser(await User.findById(req.user.id)),walletType,platformFeeRate:feeAppliesToRole(req.user.role)?USER_PLATFORM_FEE_RATE:0,platformFeePercent:feeAppliesToRole(req.user.role)?USER_PLATFORM_FEE_RATE*100:0,paystackEnvironment:Paystack.environment()});
}catch(e){res.status(e.status||500).json({message:e.message});}});
...
app.get('/api/wallet/banking/summary', auth, async (req,res)=>{try{
  ...
  const balances=walletVisibleBalances(wallet);
  res.json({role:req.user.role,walletType,currency:wallet.currency,balance:walletAmount(wallet,wallet.currency),ngnBalance:Number(balances.NGN||0),usdBalance:Number(balances.USD||0),balances,operations:operations.map(publicBankOperation),platformFeePercent:feeAppliesToRole(req.user.role)?USER_PLATFORM_FEE_RATE*100:0,paystackEnvironment:Paystack.environment()});
}catch(e){res.status(e.status||500).json({message:e.message});}});
...
app.post('/api/wallet/banking/convert', auth, async (req,res)=>{try{
  ...
  if (!Number.isFinite(converted) || converted <= 0) return res.status(502).json({message:'The live exchange-rate service returned an invalid conversion.'});
  const reference=`CONVERT-${Date.now()}-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
  changeWalletAmount(wallet,from,-totalDebit);changeWalletAmount(wallet,to,converted);
  wallet.transactions.push({type:'debit',amount:totalDebit,currency:from,reference,description:`Converted ${amount} ${from} to ${converted} ${to}; fee ${fee} ${from}`,status:'completed'});
  wallet.transactions.push({type:'credit',amount:converted,currency:to,reference,description:`Currency conversion from ${from}`,status:'completed'});
  await wallet.save();
...
app.get('/api/admin/finance/config',auth,requireAdmin,async(_req,res)=>res.json({
  ...
  domainFirstYearMarkupUSD:0,domainRenewalMarkupUSD:0,domainRenewalMarkupUSD:DOMAIN_RENEWAL_MARKUP_USD,domainRenewalMarkupRate:0,domainRenewalMarkupPercent:0,resellerDomainApiMarkupUSD:0,resellerDomainApiPlatformFeePercent:DOMAIN_RESELLER_API_PLATFORM_FEE_RATE*100,

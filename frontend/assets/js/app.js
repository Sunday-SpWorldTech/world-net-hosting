...
let currencyConfig = { countryCurrency: {}, supportedPaystackCheckoutCurrencies: ['NGN'], displayCurrencies: ['USD','NGN','GHS','KES','ZAR','GBP','EUR'], paystackDefaultCurrency: 'NGN' };
...
async function fetchDomainApi(path,options={}){
  ...
  const error=new Error('The domain API could not be reached. Confirm the backend Vercel service is live and that its CORS FRONTEND_URL includes this website.');
...
function ensureDashboardCartButton(){
  ...
  if(page==='admin-dashboard.html' || page==='staff.html' || page==='staff.htm') return;
...
function getToken(){ return localStorage.getItem('worldnet_token') || localStorage.getItem('worldnet_admin_token'); }
function getAdminToken(){ return localStorage.getItem('worldnet_admin_token'); }
function logoutUser(){ ['worldnet_token','worldnet_user','worldnet_pin_mode','worldnet_pin_ok','worldnet_return_to'].forEach(k=>localStorage.removeItem(k)); sessionStorage.removeItem('worldnet_return_to'); location.replace('reseller-signin.html'); }
function logoutAdmin(){ ['worldnet_admin_token','worldnet_admin_user'].forEach(k=>localStorage.removeItem(k)); location.replace('admin-login.html'); }
function requireToken(){ if(!getToken()){ sessionStorage.setItem('worldnet_return_to', location.pathname.split('/').pop() || 'reseller-dashboard.html'); location.replace('reseller-signin.html'); return false; } return true; }
...
async function changeCurrency(code){
  ...
  if(preferredCurrency!=='USD'){
    ...
    try{
      const r=await fetchDomainApi(`/currency/convert?amount=1&from=USD&to=${encodeURIComponent(preferredCurrency)}`,{cache:'no-store'});
...
async function initCurrency(){
  try{
    const res = await fetchDomainApi('/currency/config',{cache:'no-store'});
    ...
    localStorage.setItem('worldnet_currency', preferredCurrency);
    if(preferredCurrency !== 'USD'){
      const rateRes = await fetchDomainApi(`/currency/convert?amount=1&from=USD&to=${encodeURIComponent(preferredCurrency)}`,{cache:'no-store'});
...
function paystackCurrency(){ const c=String(preferredCurrency||'NGN').toUpperCase(); return (currencyConfig.supportedPaystackCheckoutCurrencies||[]).includes(c)?c:(currencyConfig.paystackDefaultCurrency||'NGN'); }
...
function renderCart(){
 ...
 el.innerHTML=cart.map((i,idx)=>`<div class="domain-row"><div><strong>${i.name}</strong><br><span class="small">${i.type}</span></div><div><strong>${dualMoney(i.usdPrice ?? i.price)}</strong> <button class="btn" onclick="cart.splice(${idx},1);localStorage.setItem('world_net_hosting_cart',JSON.stringify(cart));renderCart()">Remove</button></div></div>`).join('')+`<div class="notice">Total: <strong>${dualMoney(total)}</strong><br><span class="small">Secure checkout will charge in ${paymentCurrency}. Only currencies enabled for this banking account are available.</span></div><div style="display:flex;gap:12px;flex-wrap:wrap"><button class="btn" onclick="createOrder()">Save Order</button><button class="btn teal" onclick="payWithPaystack()">Pay Now</button><button class="btn teal" onclick="payWithWallet()">Pay with Banking Balance</button><span id="checkout-wallet-balance" class="small">Loading wallet balance…</span></div>`;
}
...
async function payWithPaystack(){
 if(!getToken()){ sessionStorage.setItem('worldnet_return_to','dashboard-cart.html'); showToast('Please sign in or create an account before payment.','warning'); setTimeout(()=>location.href='reseller-signin.html',700); return; }
 ...
 try{
   ...
   let data; try{ data=raw ? JSON.parse(raw) : {}; }catch{ data={message:raw || `Payment provider returned HTTP ${res.status}`}; }
   if(!res.ok) throw new Error(data.message || 'Payment initialization failed');
   if((data?.data?.link||data?.data?.authorization_url)){ localStorage.setItem('worldnet_pending_payment','yes'); location.assign(data.data.link||data.data.authorization_url); return; }
...
async function authRequest(path, payload){
  try {
    ...
    if (/404|route unavailable|route not found/i.test(String(error?.message || ''))) {
      throw new Error(`Authentication endpoint was not found at ${API_BASE}${path}. Start the backend on port 10000 locally or configure VITE_API_BASE_URL in the frontend environment.`);
    }
    throw error;
  }
}
...
async function handleAdminLogin(event){
  ...
  if(status){status.style.display='block';status.textContent='Checking administrator PIN securely…';}
  if(!/^\d{6,12}$/.test(pin)){
    if(status)status.textContent='Enter the configured 6–12 digit administrator PIN.';
    return;
  }
  setFormBusy(form,true,'Signing in…');
  try{
    const data=await authRequest('/auth/admin/login',{pin});
    if(!data?.token||data?.user?.role!=='admin') throw new Error('The backend did not return a valid administrator session.');
    saveAdminSession(data);
    ['worldnet_token','worldnet_user','worldnet_pin_mode','worldnet_pin_ok'].forEach(key=>localStorage.removeItem(key));
    form.reset();
    if(status)status.textContent=data.message||'Administrator login successful.';
    location.replace('admin-dashboard.html');
  }catch(error){
    if(status)status.textContent=error.message||'Administrator login failed.';
  }finally{
...
async function handlePin(event){
  ...
  try{const payload=Object.fromEntries(new FormData(form));const data=await authTokenRequest(mode==='create-pin'?'/auth/pin/create':'/auth/pin/verify',payload);localStorage.setItem('worldnet_pin_ok','yes');localStorage.setItem('worldnet_user',JSON.stringify(data.user||JSON.parse(localStorage.getItem('worldnet_user')||'{}')));const savedUser=JSON.parse(localStorage.getItem('worldnet_user')||'{}');const next=sessionStorage.getItem('worldnet_return_to')||(savedUser.role==='staff'?'staff.html':'reseller-dashboard.html');sessionStorage.removeItem('worldnet_return_to');location.replace(next);}
  catch(error){
    if(String(error.message||'').includes('No PIN found')){localStorage.setItem('worldnet_pin_mode','create-pin');await setupPinPage();setStatus('Your account needs a new dashboard PIN. Create it below to continue.');}
    else setStatus(error.message||'PIN request failed.');
  }
  finally{setFormBusy(form,false);}
}
...
async function setupPinPage(){
  const form = $('#pin-form'); if(!form) return; if(!requireToken()) return;
  let mode = localStorage.getItem('worldnet_pin_mode') || '';
  try{
    const data=await apiJson(`${API_BASE}/auth/me`,{headers:{Accept:'application/json',Authorization:`Bearer ${getToken()}`}});
    mode=data.user?.hasPin?'verify-pin':'create-pin';
    localStorage.setItem('worldnet_pin_mode',mode);
    localStorage.setItem('worldnet_user',JSON.stringify(data.user||{}));
  }catch(error){
    if(!mode)mode='verify-pin';
    if(/backend|api|reach|network|timed out/i.test(String(error?.message||''))) setStatus(error.message);
  }
  ...
  if(form.dataset.pinBound!=='true'){form.dataset.pinBound='true';form.addEventListener('submit', handlePin);}
}
...
async function loadAdmin(){
 ...
 try{
   ...
   <div class="notice">Domain API: ${stats.domainApiConfigured ? 'Configured on backend environment' : 'Not fully configured. Add the Domain API values to the server environment.'} | Paystack: ${stats.paystackConfigured ? 'Configured' : 'Not configured'} | Paystack checkout currencies: ${(stats.supportedPaystackCheckoutCurrencies||[]).join(', ')}</div>
 ...
 }catch(e){el.innerHTML=`<div class="notice">${e.message}. Use admin-login.html and make sure the backend API is running.</div>`}
}
...
function setupSupportChat(){
  ...
  if(embedded) return;
  const page=(location.pathname.split('/').pop()||'').toLowerCase();
  const noChatPages=['admin-dashboard.html','staff.html','staff.htm','wallet-deposit.html','wallet-withdraw.html','wallet-send.html','wallet-transfer.html','wallet-receive.html','wallet-convert.html'];
...
async function apiJson(url, options={}){
  ...
  const suffix=rawUrl.startsWith(API_BASE)?rawUrl.slice(API_BASE.length):'';
  const urls=suffix?[...new Set(API_CANDIDATES.map(base=>`${base}${suffix}`))]:[rawUrl];
  let lastError=null;
  for(const target of urls){
    ...
    let res;
    try{
      res=await fetch(target,{cache:'no-store',...options,signal:options.signal||controller.signal});
    }catch(error){
      clearTimeout(timeout);
      lastError=error?.name==='AbortError'?new Error('The backend request timed out. Confirm the backend API is running and try again.'):new Error('Unable to reach the World Net Hosting backend. Confirm the backend API is running and the frontend API URL is correct.');
      continue;
    }
    clearTimeout(timeout);
    if(res.status===404&&urls.length>1){lastError=new Error(`Backend route unavailable at ${target}`);continue;}
    const contentType=String(res.headers.get('content-type')||'').toLowerCase();
    const text=await res.text();
    let data={};
    if(text && contentType.includes('application/json')){
      try{data=JSON.parse(text);}catch{throw new Error(`The backend returned invalid JSON (HTTP ${res.status}).`);}
    }else if(text){
  ...
  throw lastError||new Error('Unable to reach the World Net Hosting backend.');
}
...
function protectPrivatePages(){
  ...
  const userPages=['reseller-dashboard.html','dashboard-cart.html','dashboard-register-domain.html','dashboard-dns-manager.html','dashboard-business-email.html','dashboard-transfer-domain.html','dashboard-receive-domain.html','dashboard-support.html','dashboard-wallet.html','wallet-deposit.html','wallet-withdraw.html','wallet-send.html','wallet-transfer.html','wallet-receive.html','wallet-convert.html','dns-manager.html','domain-transfer.html','domain-receive.html'];
  if(userPages.includes(page)){
    if(!getToken()){ sessionStorage.setItem('worldnet_return_to',page); location.replace('reseller-signin.html'); return false; }
...
function publicNavigationLinks(){
  return `<details class="nav-dropdown"><summary class="nav-item">Domains <span>⌄</span></summary><div class="dropdown-menu"><a href="domains.html">Domain Search</a><a href="dns-manager.html">DNS Manager</a><a href="domain-transfer.html">Transfer Domain</a><a href="domain-receive.html">Receive Domain</a><a href="reviews.html">Reviews</a><a href="deals.html">Deals</a><a href="security.html">Security</a><a href="terms.html">Terms and Conditions</a><a href="policy.html">Policy</a><a href="help.html">Help</a></div></details><a href="services.html" class="nav-item">Services</a><a href="email.html" class="nav-item">Email</a><a href="banking.html" class="nav-item">Banking</a><a href="pricing.html" class="nav-item">Pricing</a><a href="about.html" class="nav-item">About</a><a href="contact.html" class="nav-item">Contact</a><a href="ai-builder.html" class="nav-item">AI Builder</a><a href="reseller-api.html" class="nav-item">Reseller API</a>`;
}
function ensureCompletePublicNavigation(){
  ...
  header.innerHTML=`<a class="logo" href="index.html"><img class="site-logo" src="images/logo1.jpeg" alt="World Net Hosting logo"><span class="site-brand-name">WNH</span></a><button type="button" class="nav-toggle" aria-label="Open navigation menu" aria-expanded="false"><span class="nav-toggle-icon">☰</span><span class="nav-toggle-label">Menu</span></button><div class="nav-left">${publicNavigationLinks()}</div><div class="nav-right"><a href="reseller-signin.html" class="nav-item nav-auth-link">Joinfree</a><a href="cart.html" class="cart" aria-label="Cart" data-count="0">🛒</a></div>`;
  document.body.prepend(header);
}
function normalizePublicNavigation(){
  if(document.body.classList.contains('dashboard-body')||document.documentElement.classList.contains('dashboard-embedded')) return;
  const header=document.querySelector('.top-nav'); if(!header)return;
  const navLeft=header.querySelector('.nav-left'); if(navLeft)navLeft.innerHTML=publicNavigationLinks();
}
  ...
  document.querySelectorAll('[data-wallet-access]').forEach(link=>{
    link.addEventListener('click',event=>{
      ...
      if(!getToken()){
        ...
        location.href=`reseller-signin.html?next=${encodeURIComponent(target)}`;
      }
    });
  });
}
function secureDashboardLinks(){
  document.querySelectorAll('a[href="reseller-dashboard.html"],a[data-dashboard-link]').forEach(a=>a.addEventListener('click',e=>{
    if(!getToken()){ e.preventDefault(); sessionStorage.setItem('worldnet_return_to','reseller-dashboard.html'); location.href='reseller-signin.html'; }
    else if(localStorage.getItem('worldnet_pin_ok')!=='yes'){ e.preventDefault(); sessionStorage.setItem('worldnet_return_to','reseller-dashboard.html'); location.href='pin.html'; }
  }));
}
...
function injectAccountActions(){
  ...
  auth.href='reseller-signin.html';
...
async function setupAiBuilder(){
  ...
  form.addEventListener('submit',async event=>{
    event.preventDefault();
    const aiToken=getToken()||getAdminToken();
    if(!aiToken){ const target=location.pathname.split('/').pop()+location.search; sessionStorage.setItem('worldnet_return_to',target); localStorage.setItem('worldnet_return_to',target); location.href=`reseller-signin.html?next=${encodeURIComponent(target)}`; return; }
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  try { updateCartCount(); loadPlans(); renderCart(); loadAdmin(); loadDashboard(); setupPinPage(); setupSupportChat(); } catch (error) { console.warn('Optional page initialization skipped:', error); }
...
async function initializeWalletDeposit(event){
  ...
  button.disabled=true; button.textContent='Connecting...'; out.textContent='A 4% platform service fee will be added. Connecting securely to the payment provider...';
  ...
  try{
    ...
    if(!(data?.data?.link||data?.data?.authorization_url)) throw new Error(data.message||'Payment provider did not return a checkout link.');
    out.textContent='Redirecting to secure payment...';
    window.location.assign(data.data.link||data.data.authorization_url);
  }catch(e){out.textContent=e.name==='AbortError'?'Payment provider connection timed out. Please check the backend and try again.':e.message;button.disabled=false;button.textContent='Deposit';}
...
async function initializeSystemWalletDeposit(event){
  ...
  button.disabled=true; button.textContent='Connecting...'; out.textContent='Connecting securely to the payment provider...';
  try{
    ...
    if(!(data?.data?.link||data?.data?.authorization_url)) throw new Error(data.message||'Payment provider did not return a checkout link.');
    out.textContent='Redirecting to secure payment...'; window.location.assign(data.data.link||data.data.authorization_url);
...
async function loadWalletBanking(){
 ...
 host.innerHTML=`<div><p class="app-kicker">Banking tools</p><h2>Transfer, Receive & Convert</h2><p class="small">${summary.role==='admin'?'Administrator operations use the provider settlement amount without platform markup.':`${fee}% platform fee applies to transfers, received funds and conversions.`}</p></div>
 <div class="banking-forms">
 <form id="bank-transfer-form" class="banking-form"><h3>Transfer to Bank</h3><input name="amount" type="number" min="100" step="0.01" placeholder="Amount" required><select name="currency"><option>NGN</option><option>GHS</option><option>KES</option><option>ZAR</option></select><select name="bankCode" id="bank-transfer-bank" required><option value="">Select bank</option></select><input name="accountNumber" inputmode="numeric" placeholder="Account number" required><input name="accountName" placeholder="Resolved account name" readonly required><textarea name="reason" rows="2" placeholder="Transfer reason"></textarea><span class="fee-inline">Fee: ${fee}% · Bank payout capability must be enabled.</span><button class="btn teal">Transfer</button><div class="small form-status"></div></form>
 <div class="banking-form receive-bank-card"><h3>Receive from Any Bank</h3>${summary.role==='admin'?`<div class="receive-account">Use your configured business settlement/collection account. Admin receives at the real API amount.</div>`:dva.active&&dva.accountNumber?`<div class="receive-account bank-account-live"><div><span>Bank Name</span><b>${escapeHtml(dva.bankName||'Paystack partner bank')}</b></div><div><span>Account Number</span><b id="user-receive-account-number">${escapeHtml(dva.accountNumber)}</b></div><div><span>Account Name</span><b>${escapeHtml(dva.accountName||'')}</b></div><div><span>Currency</span><b>${escapeHtml(dva.currency||'NGN')}</b></div></div><div class="bank-account-copy-actions"><button id="copy-receive-account" class="btn" type="button">Copy Account Number</button><button id="copy-receive-name" class="btn" type="button">Copy Account Name</button><button id="copy-receive-bank" class="btn" type="button">Copy Bank</button></div><p class="small">The account name shown is the live name returned by Paystack. Paystack normally formats dedicated accounts as Product Name / Customer Name; custom naming requires Paystack approval.</p>`:`<div class="receive-account"><b>Get your dedicated account number</b><br><span class="small">Use this account to receive bank transfers into your World Net Hosting wallet.</span></div><form id="receive-account-form" class="receive-account-form"><input name="phone" autocomplete="tel" placeholder="Phone number (e.g. +234...)" required><label class="auth-check"><input name="consent" type="checkbox" required> I consent to creating a dedicated receiving account through the configured banking provider.</label><button class="btn teal" type="submit">Get Account Number</button></form>`}<div id="receive-account-status" class="small">${!dva.active&&dva.assignmentMessage?escapeHtml(dva.assignmentMessage):''}</div></div>
 ...
 tf?.addEventListener('submit',submitBankTransfer);document.getElementById('wallet-convert-form')?.addEventListener('submit',submitWalletConvert);document.getElementById('receive-account-form')?.addEventListener('submit',createReceiveAccount);document.getElementById('copy-receive-account')?.addEventListener('click',()=>{const n=document.getElementById('user-receive-account-number')?.textContent?.trim();if(n)navigator.clipboard?.writeText(n);});document.getElementById('copy-receive-name')?.addEventListener('click',()=>{const v=dva.accountName||'';if(v)navigator.clipboard?.writeText(v);});document.getElementById('copy-receive-bank')?.addEventListener('click',()=>{const v=dva.bankName||'';if(v)navigator.clipboard?.writeText(v);});
...
async function createReceiveAccount(event){event?.preventDefault?.();const form=event?.currentTarget||document.getElementById('receive-account-form'),out=document.getElementById('receive-account-status');if(!form||!out)return;const button=form.querySelector('button');const body=Object.fromEntries(new FormData(form));body.consent=Boolean(form.querySelector('[name=consent]')?.checked);button.disabled=true;out.textContent='Creating your dedicated account number…';try{const d=await apiJson(`${API_BASE}/wallet/banking/receive-account`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${getToken()}`},body:JSON.stringify(body)});out.textContent=d.message;await loadWalletBanking();}catch(x){out.textContent=x.message;}finally{button.disabled=false;}}
...
document.addEventListener('DOMContentLoaded',()=>{ensureCompletePublicNavigation();normalizePublicNavigation();setupResponsiveNavigation();setupWalletAccessLinks();updatePublicAuthNavigation();updateCartCount();});
window.addEventListener('pageshow',event=>{if(event.persisted){try{loadDashboard();loadAdmin();loadWalletBanking();updatePublicAuthNavigation();updateCartCount();}catch(_){}}});

...
async function loadDashboard(){
 ...
 try{ const res=await fetch(`${API_BASE}/user/dashboard`,{headers:{Authorization:`Bearer ${getToken()}`}}); const data=await readApiResponse(res); if(!res.ok) throw new Error(data.message || `Dashboard request failed (${res.status})`); el.className=''; el.innerHTML=`<div class="cards dashboard-stats-grid"><div class="card dashboard-stat-card"><h3>Welcome</h3><p>${escapeHtml(data.user.name)}</p><span class="badge">${escapeHtml(data.user.email)}</span></div><div class="card dashboard-stat-card"><h3>Real Banking Balance</h3><div class="price">${money(data.wallet?.balance || 0, data.wallet?.currency || 'NGN')}</div></div><div class="card dashboard-stat-card"><h3>Orders</h3><div class="price">${data.summary.orders}</div></div><div class="card dashboard-stat-card"><h3>Registered Domains</h3><div class="price">${(data.domains||[]).length}</div></div></div><h2 style="margin-top:24px">My Domains & DNS</h2>${(data.domains||[]).length ? data.domains.map(d=>`<div class="domain-row"><div><strong>${escapeHtml(d.domain)}</strong><br><span class="small">${escapeHtml(d.status)} • ${(d.nameservers||[]).map(escapeHtml).join(', ')}</span></div><div><button class="btn teal" onclick="openDnsManager('${String(d.domain).replace(/'/g,"\\'")}')">Manage DNS</button></div></div><div id="dns-${d.domain.replace(/[^a-z0-9]/gi,'-')}"></div>`).join('') : '<div class="notice">No registered domain yet. Complete payment for a domain to add it here.</div>'}<h2 style="margin-top:24px">Recent Orders</h2>${data.orders.length ? data.orders.map(o=>`<div class="domain-row"><div><strong>Order ${escapeHtml(o._id)}</strong><br><span class="small">${new Date(o.createdAt).toLocaleString()} • Domain: ${escapeHtml(o.domainProvisionStatus||'not_started')}</span></div><div><strong>${money(o.total,o.currency || 'USD')}</strong> <span class="badge">${escapeHtml(o.status)}</span></div></div>`).join('') : '<div class="notice">No order yet.</div>'}`; }catch(e){ el.textContent=e.message; }
}
...
function protectPrivatePages(){
  ...
  const userPages=['reseller-dashboard.html','dashboard-cart.html','dashboard-register-domain.html','dashboard-dns-manager.html','dashboard-business-email.html','dashboard-transfer-domain.html','dashboard-receive-domain.html','dashboard-support.html','dashboard-wallet.html','card-issue.html','wallet-deposit.html','wallet-withdraw.html','wallet-send.html','wallet-transfer.html','wallet-receive.html','wallet-convert.html','dns-manager.html','domain-transfer.html','domain-receive.html'];
